...
首次取得七天班表，另抓第八天作為最後一天的跨日銜接，保留前日列車資料。高鐵採滾動快取：已取得的日期直接沿用，正常每天只補新增的一天，通常一次資料請求（每日期一頁，超過 1,000 筆才分頁）；重跑同一天不重抓已完整取得的日期。缺漏或先前更新失敗的日期會重試，因此首次通常八次，漏跑多天後請求也會增加。所有訪客共用。實際次數記在更新紀錄；一次上限 60 次、請求間隔至少 13 秒。首次更新缺少昨日資料時，會提示凌晨銜接可能不完整。可設定 `TRA_SOURCE=tdx` 改走 TDX 台鐵來源，但會增加請求，通常不需要。
...
`npm run data:metro` 每日抓一次 TDX StationTimeTable、StoppingPattern、S2STravelTime，共 3 次請求，原子更新 `public/data/metro.json`；失敗保留最後成功檔案，舊日期顯示提醒。每日 Actions 已串接，連同滾動補齊高鐵通常每日共 4 次 TDX 請求；首次或強制更新約 11 次。
