...
- 台鐵、高鐵與機捷的轉乘間隔上下限皆可設定，含邊界時間；預設台鐵 5–30 分鐘、高鐵與機捷 10–40 分鐘，儲存後立即重算。
...
對向可選 22 個已通車機捷站（A1–A22，含 A14a，不含未通車 A14），去回程經高鐵新竹、桃園及機捷 A18。機捷不提供車種篩選，自動包含可搭直達車與普通車。接駁上下限獨立儲存在本機，不傳給 GA。A18 本身只含步行。
...
機捷發車時間來自官方站別班表；抵達時間由發車時間加官方 OD 旅行時間預估，介面明確標示，沒有虛構車次編號。僅計算 A18 與選定站間同車可達的班次，依方向、終點與停靠模式判斷，不把未停 A18 的直達車列入；不另計機捷內部換車。桃園高鐵／A18 接駁預設 10–40 分鐘，包含上下限。三種運具各自設定 1–180 的整數分鐘，下限不得大於上限。舊偏好補入原有預設下限並保留有效上限；清除偏好會還原預設值。
