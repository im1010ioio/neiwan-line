...
function footer() {
    return `<footer><span>沿著鐵道，安排下一段旅程。</span><div><a href="#privacy">隱私權與使用條款</a><button class="text-button" id="privacy-settings">隱私設定</button><a href="https://www.railway.gov.tw/tra-tip-web/tip/tip001/tip112/gobytime" target="_blank" rel="noreferrer">台鐵官方查詢 ↗</a><a href="https://www.thsrc.com.tw/" target="_blank" rel="noreferrer">高鐵官方查詢 ↗</a></div><p>班表來源：臺鐵官方開放資料／TDX。每日更新，非即時誤點資訊；實際運行與座位請以官方資訊為準。</p></footer>`;
}
...
function privacyPage() {
    return `<main class="privacy">
        ...
        <h3>網站連線與第三方服務</h3>
        <p>起迄站篩選與轉乘計算在瀏覽器內完成，不會直接向 TDX 傳送您的查詢。瀏覽器仍會向網站主機 GitHub Pages 讀取所選日期的靜態班表檔案，並向 font.emtech.cc 載入 LINESeedTW 字體。主機與字體服務可能處理 IP 位址、請求時間、檔案路徑等一般連線紀錄；這與 GA 分析不同。</p>
        <p>前往台鐵、高鐵、Google、Instagram 等外部網站後，適用各網站的隱私政策。</p>
        ...
        <hr>
        <h2>免責聲明</h2>
        <p>本網站依據臺鐵官方開放資料及 TDX 提供的班表，搭配您選擇的條件計算轉乘組合，結果僅供旅程規劃參考。本網站並非台鐵或高鐵官方網站，也不提供訂票或座位保證。</p>
        <p>班表原則上每日更新，不提供即時誤點資訊。我們會盡力維持資料與計算結果的準確性，但不保證所有資訊均為最新、完整或完全沒有錯誤，也不保證網站持續可用。</p>
        <p>轉乘組合不代表保證接得上下一班車。實際發車、停駛、臨時調整、月台、步行時間及座位狀況，請以台鐵、高鐵與現場公告為準，並自行預留足夠的轉乘時間。</p>
